// Generate platform icons from build/icon-source.png (the Catnap app icon).
// Output:
...
const src = join(buildDir, "icon-source.png");
...
await sharp(src).resize(1024, 1024).png({ compressionLevel: 9 }).toFile(main);
console.log(`✓ ${main}`);
